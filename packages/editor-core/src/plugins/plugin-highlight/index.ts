...
export const highlightSchema = $markSchema('mark', () => ({
  attrs: { color: { default: DEFAULT_COLOR, validate: 'string' } },
  parseDOM: [{ tag: 'mark', getAttrs: (node: HTMLElement) => ({ color: node.style.backgroundColor }) }],
  ...
  toMarkdown: {
    ...
    runner: (state, mark) => {
      ...
      if (color?.toLowerCase() === DEFAULT_COLOR.toLowerCase()) color = undefined;
      state.withMark(mark, 'mark', undefined, { data: { color } });
    },
  },
}));
...
export const highlightInputRule = $inputRule((ctx) =>
  markRule(/(?:==)(?:\{([^}]+)\})?([^=]+)(?:==)$/, highlightSchema.type(ctx), {
    getAttr: (match) => ({ color: match[1] || null }),
  }),
);
...
export { ColorPickerTooltip } from './view';
export type { ColorOptions } from './view/ColorPicker';

...
export const toggleColorPickerCommand = $command('ToggleColorPicker', (ctx) => () => (state) => {
  const {
    doc,
    selection: { from, to },
  } = state;
  ...
  doc.nodesBetween(from, to, (node) => {
    ...
    mark = node.marks.find((item) => item.type === markType) ?? null;
  });
  ctx.get(colorPickTooltipAPI.key).addColorPicker(mark, from, to);
  return true;
});

export const showColorPickerCommand = $command('ToggleColorPicker', (ctx) => () => (state) => {
  const {
    doc,
    selection: { from, to },
  } = state;

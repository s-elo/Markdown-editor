...
interface Data { from: number; to: number; mark: Mark | null; }
...
export class ColorPickerTooltip implements PluginView {
  ...
  constructor(private readonly ctx: Ctx, view: EditorView) {
    ...
    this.#root.render(createElement(ColorPicker, { onColorChange: this.#onColorChange, initialColor: '', colorOptions: [] }));
    this.#provider = new TooltipProvider({ content, floatingUIOptions: { placement: 'bottom' }, debounce: 0, shouldShow: () => false });
    this.#provider.onHide = () => requestAnimationFrame(() => view.dom.focus({ preventScroll: true }));
  ...
  #reset = () => { this.#provider.hide(); this.#data = { ...defaultData }; };

  #onColorChange = (color: string) => {
    const { from, to, mark } = this.#data;
    if (color === 'default') { this.#reset(); this.remove(from, to); return; }
    const view = this.ctx.get(editorViewCtx);
    const type = highlightSchema.type(this.ctx);
    if (mark?.attrs?.color === color) { this.#reset(); return; }
    const tr = view.state.tr;
